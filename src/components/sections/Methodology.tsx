...
export default function Methodology() {
 ...
 icon: <Search className="w-6 h-6 text-ember" />
 },
 {
 id: "02",
 title: t('steps.02.title'),
 desc: t('steps.02.desc'),
 icon: <TrendingUp className="w-6 h-6 text-ember" />
 },
 {
 id: "03",
 title: t('steps.03.title'),
 desc: t('steps.03.desc'),
 icon: <ChefHat className="w-6 h-6 text-ember" />,
 cta: t('steps.03.cta')
 }
 ];
 ...
 {steps.map((step, idx) => (
 <ScrollReveal key={idx} mode="slide-up" delay={idx * 0.2} className="relative z-10 group bg-surface/50 p-8 rounded-3xl border border-white/5 backdrop-blur-sm hover:border-ember/50 transition-colors duration-500 flex flex-col items-start h-full">
 <div className="w-16 h-16 rounded-2xl bg-cream border border-white/10 flex items-center justify-center mb-8 group-hover:bg-ember/10 group-hover:scale-110 transition-all duration-500 shadow-xl relative">
 {step.icon}
 <span className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-ember text-ink font-black text-xs flex items-center justify-center shadow-[0_0_15px_rgba(212,255,0,0.5)]">
 {step.id}
 </span>
 </div>
 ...
 className="mt-6 text-ember font-bold text-sm uppercase tracking-widest hover:underline decoration-2 underline-offset-4"
 >
 {step.cta}
 </button>
 )}
