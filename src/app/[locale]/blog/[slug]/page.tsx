...
export default async function BlogPostPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
    ...
    const components = {
        ...
        a: (props: any) => <a className="text-cyan underline decoration-cyan/30 underline-offset-4 hover:text-white hover:decoration-white transition-all" {...props} />,
        blockquote: (props: any) => <blockquote className="border-l-4 border-acid-lime pl-8 py-4 my-10 font-serif italic text-2xl text-white/90 bg-white/5 rounded-r-xl" {...props} />,
    };

    return (
        <article className="min-h-screen bg-obsidian text-white pt-32 pb-32 px-6 relative">
            <ReadingProgress />
            ...
            <div className="max-w-3xl mx-auto">
                ...
                <Link
                    href={`/${locale}/blog`}
                    className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-white/40 hover:text-acid-lime mb-12 transition-colors group"
                >
                ...
                <header className="mb-16">
                    <div className="flex gap-3 mb-8">
                        {post.translation.tags?.map(tag => (
                            <span key={tag} className="text-xs font-bold text-obsidian bg-acid-lime uppercase tracking-widest px-3 py-1 rounded-full">
                                {tag}
                            </span>
                        ))}
                    </div>
