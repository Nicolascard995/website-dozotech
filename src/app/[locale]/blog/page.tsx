...
export default async function BlogPage({ params }: { params: Promise<{ locale: string }> }) {
    ...
    return (
        <main className="min-h-screen bg-obsidian text-white pt-32 pb-20 px-6">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    {posts.map((post) => (
                        <Link
                            ...
                            className="group block bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-acid-lime/50 transition-all"
                        >
                            ...
                            <div className="p-8">
                                <div className="flex gap-2 mb-4">
                                    {post.translation?.tags?.map(tag => (
                                        <span key={tag} className="text-xs font-bold text-acid-lime uppercase tracking-widest border border-acid-lime/20 px-2 py-1 rounded">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
                                <h2 className="text-2xl font-bold font-display mb-3 group-hover:text-acid-lime transition-colors">
